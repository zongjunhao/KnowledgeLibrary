...
<!-- 						</div> -->
 	...
 	$(function(){
 		search_msg=(getUrlParam("search_msg")==null?"":getUrlParam("search_msg"));
 		order=(getUrlParam("order")==null?"k_click":getUrlParam("order"));
 		searchKnowledges(search_msg,f_id,order)
 	})
 	...
 	$(".panel-heading").text("知识列表");
 	$(".search").click(function(){
 		order=$(".select option:selected").val()
 		searchKnowledges(search_msg,f_id,order);
 	})
    var table=$('#dataTables-example').dataTable();
 	$(".table tbody").empty();
    function searchKnowledges(search_msg,f_id,order){
    	table.fnDestroy()
    	$.ajax({
 	 		type:"POST",
 	 		...
 	 		success:function(res){
 	 			...
 	 			if(res.resultCode=="2006"){//查找成功
 	 				...
 	 				$.each(res.data, function(i,value) {
 	 				    ...
 	 				    "<td>"+(i+1)+"</td>"+
 	 				    "<td class='knowledge' id="+value.knowledge.k_id+">"+value.k_name+"</td>"+
 	 				...
 	 				$(".knowledge").click(function(){
 	 					...
 	 					location.href="knowledge_detail.jsp?id="+$(this).attr('id')+"&return=knowledge_searchAll.jsp";
 	 				});
 	 				isShow();
 	 			}else{//查找失败
 	 				isShow();
    ...
    function checkIcon(index,totalPage){
    	if(index<totalPage){
    		$("#next").attr('src',"./image/右箭头1.png");
    	}else{
    		$("#next").attr('src',"./image/右箭头.png");
    	}
    	if(index>1){
    		$("#prev").attr('src',"./image/左箭头1.png");
    	}else{
    		$("#prev").attr('src',"./image/左箭头.png");
    	}
    }
    ...
    $("#knowledge").click(function(){location.href="knowledge_detail.jsp"})
